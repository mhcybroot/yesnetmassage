...
public class BanglaCounsellingGroup extends AppCompatActivity {
            ...
            database.getReference().child("users").child(auth.getUid()).child("post").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                ...
                public void onComplete(@NonNull Task<DataSnapshot> task) {
                    if (!task.isSuccessful()) {
                        ...
                        if (post.equals("admin") || post.equals("BanglaCounsellorGroupAdmin")) {
                            database.getReference().child("group").child("BanglaCounsellingGroup").limitToLast(50).addValueEventListener(new ValueEventListener() {
                                @Override
                            ...
                            database.getReference().child("group").child("BanglaCounsellingGroup").addValueEventListener(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    ...
                                    for (DataSnapshot snapshot1 : snapshot.getChildren()) {
                                        massageModels model1 = snapshot1.getValue(massageModels.class);
                                        if (model1.getuId().equals("yIDjlCBwnEaDWeklbJr4CwwVGrs2")) {
                                            model1.setMassageID(snapshot.getKey());
                                            MassageModels.add(model1);
                                        }
            ...
            binding.chatdeeatlsSendButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (massage.isEmpty()) {
                        ...
                        final massageModels model = new massageModels(auth.getUid(), massage, false);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.Member:
                database.getReference().child("group").child("BanglaCounsellingGroup").child("GroupInfo").child("memberlist").child(Objects.requireNonNull(auth.getUid())).child("post").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<DataSnapshot> task) {
                        if (task.isSuccessful()) {
                            String post = String.valueOf(task.getResult().getValue());
                            ...
                            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                                reference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                    ...
                                    public void onSuccess(Uri uri) {
                                        ...
                                        final massageModels model = new massageModels(auth.getUid(), imageUrl, "Photo", 1, false);
